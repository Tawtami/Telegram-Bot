...
async def test_admin_pagination_and_exports(monkeypatch):
    ...
    try:
        ...
        async with ClientSession() as sess:
            async with sess.get(f"{base}/admin?token=test-token&page=1") as r:
                assert r.status == 200
                html = await r.text()
                assert "سفارش‌ها" in html
            # CSV/XLSX links
            async with sess.get(f"{base}/admin/export.csv?token=test-token") as r2:
                assert r2.status == 200
            async with sess.get(f"{base}/admin/export.xlsx?token=test-token") as r3:
