...
async def test_admin_filters_advanced(monkeypatch):
    monkeypatch.setenv("PORT", "8097")
    ...
    try:
        ...
        async with ClientSession() as sess:
            async with sess.get(f"{base}/admin?token=test-token&type=course&status=approved") as r:
                assert r.status == 200
                html = await r.text()
                assert "course" in html and "approved" in html
    finally:
        task.cancel()
