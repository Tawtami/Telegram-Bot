...
async def test_admin_flash_messages(monkeypatch):
    ...
    with session_scope() as s:
        u = User(telegram_user_id=111111119, first_name="x", last_name="y", phone="z")
        s.add(u)
        s.flush()
    ...
    try:
        ...
        async with ClientSession() as sess:
            async with sess.get(f"{base}/admin?token=test-token&flash=ok") as r:
                assert r.status == 200
                html = await r.text()
                assert "ok" in html
    finally:
