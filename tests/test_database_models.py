...
def test_purchase_model(session):
    ...
    user = User(
        ...
        first_name_enc="test_user",
        last_name_enc="test_last"
    )
    session.add(user)
...
def test_profile_change_model(session):
    ...
    user = User(
        ...
        first_name_enc="change_user",
        last_name_enc="change_last"
    )
    session.add(user)
    session.flush()
...
def test_model_relationships(session):
    ...
    user = User(
        ...
        first_name_enc="rel_user",
        last_name_enc="rel_last"
    )
    session.add(user)
    session.flush()
...
def test_model_constraints(session):
    ...
    user1 = User(
        ...
        first_name_enc="unique_user1",
        last_name_enc="unique_last1"
    )
    session.add(user1)
    session.commit()
...
def test_model_defaults(session):
    ...
    user = User(
        ...
        first_name_enc="default_user",
        last_name_enc="default_last"
    )
