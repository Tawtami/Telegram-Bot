...
class TestSecurityUtils:
    ...
    def test_xss_patterns(self):
        ...
        # Test data URLs
        assert any(
            pattern.search("data:text/html,<script>alert('xss')</script>") for pattern in patterns
        )
        assert any(
            pattern.search("data:application/x-javascript,alert('xss')") for pattern in patterns
        )
...
<<<<<<< HEAD
        assert any(
            pattern.search("/var/log") for pattern in patterns
        )  # nosec B108 - testing pattern
        assert any(
            pattern.search("/tmp/file") for pattern in patterns
        )  # nosec B108 - testing pattern
...
=======
        ...
        assert any(pattern.search("/home/user") for pattern in patterns)  # nosec B108 - testing pattern
>>>>>>> c90ab58 (chore(tests): mark /home/user pattern with # nosec B108)
    ...
    def test_validate_filename_valid(self):
        ...
        valid_filenames = ["document.pdf", "image.jpg", "file_123.txt", "my-file.docx", "file.name"]
    ...
    def test_sanitize_json(self):
        ...
        # Test dict
        data = {"name": "John<script>alert('xss')</script>", "age": 25}
    ...
    def test_log_security_event(self):
        ...
        with patch('utils.security.logger') as mock_logger:
            SecurityUtils.log_security_event(
                event_type="login_attempt", user_id=12345, details={"ip": "192.168.1.1"}
            )
